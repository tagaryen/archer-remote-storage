...
public class SM4Util {
    ...
    private static int L_ap(
        int B)
    {
        return (B ^ (rotateLeft(B, 13)) ^ (rotateLeft(B, 23)));
    }

    private static int T_ap(
        int Z)
    {
        return L_ap(tau(Z));
    }

    private static int[] expandKey(boolean forEncryption, byte[] key)
    {
        int[] rk = new int[32];
        int[] MK = new int[4];
    ...
    private static int T(int Z)
    {
        return L(tau(Z));
    }

    private static int F0(int[] X, int rk)
    {
        return (X[0] ^ T(X[1] ^ X[2] ^ X[3] ^ rk));
    }

    private static int F1(int[] X, int rk)
    {
        return (X[1] ^ T(X[2] ^ X[3] ^ X[0] ^ rk));
    }

    private static int F2(int[] X, int rk)
    {
        return (X[2] ^ T(X[3] ^ X[0] ^ X[1] ^ rk));
    }

    private static int F3(int[] X, int rk)
    {
        return (X[3] ^ T(X[0] ^ X[1] ^ X[2] ^ rk));
    }

    public static int processBlock(byte[] in, int inOff, byte[] out, int outOff, int[] rk)
    ...
    private static void pkcs7Padding(byte[] f, int r) {
        byte b = (byte) (BLOCK_SIZE - r);
        while(r < BLOCK_SIZE) {
            f[r++] = b;
        }
    }
    ...
    public static byte[] encrypt(byte[] plaintext, byte[] key) {
    	if(key.length != BLOCK_SIZE) {
    		throw new IllegalArgumentException("invalid sm4Key");
    	}
    	int[] kr = expandKey(true, key);
    	int r =  plaintext.length % BLOCK_SIZE, d = plaintext.length / BLOCK_SIZE;
    	byte[] out = new byte[plaintext.length + BLOCK_SIZE - r];
        for (int i = 0; i < d; ++i) {
        	processBlock(plaintext, i * BLOCK_SIZE, out, i * BLOCK_SIZE,  kr);
        }
        byte[] f = new byte[BLOCK_SIZE];
        System.arraycopy(plaintext, d * BLOCK_SIZE, f, 0, r);
        pkcs7Padding(f, r);
    	processBlock(f, 0, out, d * BLOCK_SIZE,  kr);
        return out;
    }
    ...
    public static byte[] decrypt(byte[] ciphertext, byte[] key) {
    	...
    	int[] kr = expandKey(false, key);
    	int d = ciphertext.length / BLOCK_SIZE;
    	byte[] out = new byte[ciphertext.length];
        for (int i = 0; i < d; ++i) {
        	processBlock(ciphertext, i * BLOCK_SIZE, out, i * BLOCK_SIZE, kr);
        }
        int l = out.length - out[out.length - 1];
